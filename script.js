...
const API_CARRITOS = "https://fakestoreapi.com/carts";

let CarritosAPI = [];

async function CargarCarritosAPI() {
    const contenedor = document.getElementById("carritosContainer");
    const mensaje = document.getElementById("mensajeCarritos");

    try {
        const respuesta = await fetch(API_CARRITOS);

        if (!respuesta.ok) {
            throw new Error("Error al consumir el endpoint de carritos");
        }

        CarritosAPI = await respuesta.json();

        console.log("Lista de carritos:", CarritosAPI);

        renderizarCarritosAPI();

        mensaje.textContent = "";

    } catch (error) {
        console.error("Error:", error);
        mensaje.textContent = "No se pudieron cargar los carritos.";
        contenedor.innerHTML = "";
    }
}

function renderizarCarritosAPI() {
    const contenedor = document.getElementById("carritosContainer");

    contenedor.innerHTML = CarritosAPI.map(carrito => {
        ...
        return `
            <tr>
                ...
                <td>${formatearFechaCarrito(carrito.date)}</td>
                <td>${carrito.products.length}</td>
...
function formatearFechaCarrito(fecha) {
    const nuevaFecha = new Date(fecha);

    return nuevaFecha.toLocaleDateString("es-NI", {
        year: "numeric",
        month: "long",
        day: "numeric"
    });
